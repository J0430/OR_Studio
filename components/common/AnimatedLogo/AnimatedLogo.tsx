...
import { AnimatedLogoProps, LogoPathsProps } from "./AnimatedLogo.types";
...
export const AnimatedLogo: React.FC<AnimatedLogoProps> = ({
  ...
  className = "",
  theme,
  highRes = false, // ✅ New prop
}) => {
  const { theme: currentTheme } = useTheme();

  const [paths, setPaths] = useState<LogoPathsProps | null>(null);
  const [animationKey, setAnimationKey] = useState(0);

  useEffect(() => {
    const fetchPaths = async () => {
      const res = await fetch(`/api/logo-paths?name=${logoName}`);
      if (!res.ok) {
        console.error("Failed to fetch logo paths");
        return;
      }
  ...
  return (
    <motion.svg
      ...
      viewBox="0 0 125 150"
      onClick={handleClick}
      shapeRendering={highRes ? "geometricPrecision" : "auto"} // ✅ Better quality on desktop
      preserveAspectRatio="xMidYMid meet" // ✅ Keep proportions
    >
      <mask id="logo-mask">
        <path d={paths.outerPath} fill="white" />
        {paths.innerPaths.map((d, i) => (
      ...
      {[paths.outerPath, ...paths.innerPaths].map((d, i) => (
        <motion.path
          key={i}
          d={d}
